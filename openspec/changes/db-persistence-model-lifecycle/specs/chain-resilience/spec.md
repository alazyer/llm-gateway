...
When creating a chain (via admin API or YAML seed), it is permitted to reference models that are currently `inactive`. The chain will have `status='degraded'` or `status='inactive'` depending on how many of its models are active.
