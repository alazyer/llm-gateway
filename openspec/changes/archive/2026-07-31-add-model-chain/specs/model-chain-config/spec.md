...
A chain entry MAY include `timeout_ms` (positive integer, default inherits from `request_timeout_ms`) and `max_retries` (non-negative integer, default inherits from `max_retries`). These values apply to every model in the chain unless overridden by a per-model entry.
