# web-ai-chat-frontend Specification

## Purpose

The Nuxt web chat client surface that drives the production Web AI Chat capability: routes prompts, sessions, and history through the `/api/ai-chat/*` production endpoints, attaches user identity, consumes the production SSE lifecycle, exposes stable UX states with localized typed-failure messaging, and restores session history across navigation.
