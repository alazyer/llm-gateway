...
The VS Code extension SHALL activate, read configuration, discover Copilot model availability, connect to the gateway, register models, and cleanly disconnect on deactivation.

#### Scenario: Extension activates with configured gateway
- **WHEN** the extension activates and has gateway URL and proxy token configuration
- **THEN** it SHALL attempt to connect to the gateway WebSocket endpoint
...
The extension SHALL use VS Code language model APIs to discover available Copilot chat models and map them to gateway model registrations.

#### Scenario: Copilot models available
- **WHEN** `vscode.lm` returns one or more chat models
- **THEN** the extension SHALL register corresponding `copilot-*` model identifiers with the gateway
...
- **THEN** the extension SHALL send an updated registration or status update to the gateway
...
The extension SHALL retry gateway WebSocket connections with exponential backoff capped at a configured maximum and SHALL re-register models after reconnecting.
...
- **THEN** it SHALL send a fresh registration before accepting gateway requests
