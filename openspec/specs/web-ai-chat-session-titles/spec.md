# web-ai-chat-session-titles Specification

## Purpose

Human-readable session titles for the Web AI Chat. A title is auto-derived from the first user prompt on session creation and is renameable by the session owner. Titles are persisted on the session row and exposed in the session list so chat history is scannable.
